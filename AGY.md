...
- **[Linear Integration & API Sync]**: Configured authentication and automated GraphQL issue synchronization with Linear workspace (`thomaspayne-pilot` / Team `PIL`). Successfully generated and synced tasks (PIL-5 through PIL-9) for completed refactors, Terminal HUD, and upcoming roadmap items. Added helper script `scripts/sync-linear.mjs`.
