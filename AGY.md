...
- **[Linear Integration & API Sync]**: Created dedicated Linear Team **Personal Website (`PW`)** and Project **Personal Website**. Configured GraphQL issue synchronization to assign tasks directly under `PW-1` through `PW-5` issue keys. Deleted temporary `PIL` placeholder tasks. Added helper script `scripts/sync-linear.mjs`.
