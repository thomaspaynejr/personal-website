...
import { Send, Clock, MapPin, Award, Laptop, Heart, MessageSquare, Plus, X, Activity, CheckCircle2, Construction, LogIn } from 'lucide-react';
...
export default function TimelineDashboard({ 
  ...
  return (
    <main className="max-w-4xl mx-auto px-6 py-10 space-y-20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {projects.map((project) => (
              <StaggerItem key={project.id}>
                <div className="p-5 border border-border-custom rounded-2xl bg-card/20 space-y-4 hover:border-action/50 transition-colors h-full">
                  ...
                  <div className="space-y-4 group">
                    ...
                    <div className="flex gap-4 border-t border-border-custom pt-4">
                      <button 
                        ...
                        disabled={!user}
                        className={`flex items-center gap-1.5 text-xs font-bold transition-colors group/heart uppercase tracking-tighter ${user ? 'text-accent hover:text-action' : 'text-accent/30 cursor-not-allowed'}`}
                      >
                      ...
                      <button 
                        ...
                        disabled={!user}
                        className={`flex items-center gap-1.5 text-xs font-bold transition-colors uppercase tracking-tighter ${user ? 'text-accent hover:text-action' : 'text-accent/30 cursor-not-allowed'}`}
                      >
                    ...
                    {activeCommentId === event.id && user && (
                      <div className="space-y-4 mt-4 animate-in fade-in slide-in-from-top-2">
                        <div className="flex gap-2">
                          ...
                          <button onClick={(e) => { e.preventDefault(); handlePostTimelineComment(event.id); }} className="p-2 bg-action text-background rounded-lg hover:opacity-90"><Send size={12} /></button>
                        </div>
                      </div>
                    )}
      ...
      {!user && (
        <FadeIn delay={0.5}>
          <section className="pt-10 border-t border-border-custom text-center">
            <Link href="/login" className="inline-flex items-center gap-2 text-xs font-bold text-action hover:underline uppercase tracking-[0.2em]">
              <LogIn size={14} />
              SIGN IN TO POST, LIKE & COMMENT _
            </Link>
          </section>
        </FadeIn>
      )}
