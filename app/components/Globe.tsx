...
export default function Globe({ activeLocation }: GlobeProps) {
  ...
  useEffect(() => {
    ...
    const isLand = (lat: number, lng: number) => {
      // Precise landmass approximation for "Planet Earth" look
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      projected.forEach(p => {
        ...
        const size = isHighlighted ? 3 : p.isLand ? 1.2 : 0.6;
        ...
        if (isHighlighted) {
          ...
          ctx.fillStyle = `rgba(255, 255, 255, ${opacity * (p.isLand ? 1 : 0.4)})`;
          ctx.shadowBlur = 0;
        }
        ctx.fill();
      });
  ...
  return (
    <div className="w-full h-full flex items-center justify-center">
      <canvas ref={canvasRef} width={600} height={600} className="w-full h-full mix-blend-screen" />
    </div>
  );
}
